...
class KeyBridgeWindow:
    def __init__(self, root):
        ...
        self.root.configure(bg="#f3f3f3") # Light grey background
    ...
    def _build_layout(self):
        ...
        SettingsTab(self.tabs) # Settings Tab
    ...
    def _init_dashboard_tab(self):
        ...
        self.lbl_cap_hint = ttk.Label(f_cap, text="Press ESC to Stop | F12 to Toggle", background="white", foreground="#888")
    ...
    def _init_wireless_tab(self):
        ...
        # Step 1
        f1 = ttk.LabelFrame(tab, text="Step 1: Prep Device", style="Card.TLabelframe", padding=15)
        f1.pack(fill="x", pady=10)
        ttk.Label(f1, text="Connect USB cable first, then click Enable.", background="white").pack(anchor="w", pady=(0,5))
        SafeButton(f1, text="Enable TCP Mode (Port 5555)", command=self.engine.enable_wireless).pack(fill="x")
        
        # Step 2
        f2 = ttk.LabelFrame(tab, text="Step 2: Connect", style="Card.TLabelframe", padding=15)
        f2.pack(fill="x", pady=10)
        
        row = ttk.Frame(f2, style="Card.TLabelframe")
        row.pack(fill="x", pady=5)
        
        self.ip_entry = ttk.Entry(row)
        self.ip_entry.pack(side="left", fill="x", expand=True)
        self.ip_entry.insert(0, "192.168.1.x")
        
        SafeButton(row, text="Scan IP", width=10, command=self._auto_find_ip).pack(side="left", padx=5)
        SafeButton(f2, text="Link Wireless", style="Accent.TButton", command=self._connect_ip).pack(fill="x", pady=10)
    ...
    def _init_tools_tab(self):
        ...
        self.macro_container = ttk.Frame(f2, style="Card.TLabelframe") # White bg for buttons
    ...
    def _refresh_devices(self):
        try:
            # [FIX] creationflags=0x08000000 (subprocess.CREATE_NO_WINDOW)
            # This prevents the black cmd window from flashing when scanning devices
            res = subprocess.run(
                [ADB_PATH, "devices"], 
                capture_output=True, 
                text=True,
    ...
    def _toggle_connect(self):
        if self.engine.running:
            ...
            if dev:
                ...
                messagebox.showwarning("Select Device", "Please select a device from the list.")
    ...
    def _auto_find_ip(self):
        ...
        if ip:
            ...
            self.log(f"IP Detected: {ip}")
        else: self.log("IP not found. Check WiFi.")
