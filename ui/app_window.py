...
class OverlayHUD:
    def __init__(self, root):
        ...
        self.win.config(bg="#d32f2f")
        self.win.withdraw()

        lbl = tk.Label(self.win, text="📱 PHONE MODE (Ctrl+F12 to Exit)", 
...
class KeyBridgeWindow:
    def __init__(self, root):
        ...
        # [OPTIMIZED] Input Queue with Batching
        self.input_queue = queue.Queue()
        self.processing_thread = threading.Thread(target=self._input_processor_loop, daemon=True)
        self.processing_thread.start()

        self._setup_styles()
        self._build_layout()
        
        self.root.after(500, self._init_hud) 
        self._start_pc_mode_listener()
        self._refresh_devices()
        self.root.after(2000, self._heartbeat)

    def _init_hud(self):
        self.hud = OverlayHUD(self.root)
    ...
    def _build_layout(self):
        ...
        tk.Button(header, text="☕ Support Dev", bg="#FFDD00", fg="black", font=("Segoe UI", 9, "bold"), relief="flat", padx=15, pady=4, command=lambda: SupportDialog(self.root)).pack(side="right")
        ...
        self.log_text = tk.Text(term_frame, height=7, bg="#1e1e1e", fg="#00ff00", font=("Consolas", 9), relief="flat", state="disabled")
    ...
    def _init_dashboard_tab(self):
        ...
        self.btn_capture = SafeButton(f_cap, text="START CAPTURE (Ctrl+F12)", style="Capture.TButton", command=self._activate_phone_mode)
        self.btn_capture.pack(fill="x", pady=(10, 5), ipady=10)
        self.lbl_cap_hint = ttk.Label(f_cap, text="Global Shortcut: Ctrl + F12 (Works in background)", background="white", foreground="#888")
        self.lbl_cap_hint.pack()
    ...
    def _toggle_connect(self):
        if self.engine.running:
            if self.capture_active: self._deactivate_phone_mode()
            self.engine.stop()
            self.notifier.stop()
    ...
    def _start_pc_mode_listener(self):
        if self.hotkey_listener: self.hotkey_listener.stop()
        self.hotkey_listener = keyboard.GlobalHotKeys({
            '<ctrl>+<f12>': lambda: self.root.after(0, self._activate_phone_mode)
        })
        self.hotkey_listener.start()
        self.log("Ready. Press Ctrl+F12 to toggle Phone Mode.")
    ...
    def _activate_phone_mode(self):
        ...
        if self.hud: self.hud.show()
        self.btn_capture.configure(text="⚠ TYPING ON PHONE... (Ctrl+F12 to Stop)", style="Red.TButton")
        self.log_text.config(bg="#330000")

        # FIX: Release Modifier keys (Windows gets stuck thinking Ctrl is down)
        try:
            self.keyboard_controller.release(keyboard.Key.ctrl)
            self.keyboard_controller.release(keyboard.Key.f12)
        except: pass

        self.input_listener = keyboard.Listener(
            on_press=self._handle_input_press, 
            on_release=self._handle_input_release,
            suppress=True
        )
        self.input_listener.start()
        self.log(">>> PHONE MODE ACTIVE <<<")

    def _deactivate_phone_mode(self):
        self.capture_active = False
        if self.input_listener:
            self.input_listener.stop()
            self.input_listener = None

        if self.hud: self.hud.hide()
        self.btn_capture.configure(text="START CAPTURE (Ctrl+F12)", style="Capture.TButton")
        self.log_text.config(bg="#1e1e1e")
        self.log("Keyboard Capture: OFF")
        self._start_pc_mode_listener()

    # --- [NEW] SMART BATCHING PROCESSOR ---
    def _input_processor_loop(self):
        """ 
        Smart Batcher:
        Waits 50ms to see if you type more keys.
        If yes -> Combines them (Batching).
        If no -> Sends what it has.
        """
        buffer = []
        
        while True:
            try:
                # Wait for a key for 50ms
                key = self.input_queue.get(timeout=0.05) 
                
                # If character, add to buffer
                if hasattr(key, 'char') and key.char:
                    buffer.append(key.char)
                else:
                    # Special key! Flush buffer first
                    if buffer:
                        self.engine.send_text("".join(buffer))
                        buffer = []
                    ...
                    elif key == keyboard.Key.enter: self.engine.send_cmd("input keyevent 66")
                    elif key == keyboard.Key.backspace: self.engine.send_cmd("input keyevent 67")
                    
            except queue.Empty:
                # 50ms passed with no new key. Flush buffer.
                if buffer:
                    self.engine.send_text("".join(buffer))
                    buffer = []

    def _handle_input_press(self, key):
        if not self.capture_active: return

        if key == keyboard.Key.esc or key == keyboard.Key.f12:
            self.root.after(0, self._deactivate_phone_mode)
            return

        # Dump to Queue immediately (No Lag)
        self.input_queue.put(key)

    def _handle_input_release(self, key):
        pass 
